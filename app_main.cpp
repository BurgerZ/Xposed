...
if (RUNNING_PLATFORM_SDK_VERSION >= 18) {
	*(void **) (&PTR_atrace_set_tracing_enabled) = dlsym(RTLD_DEFAULT,
	...
	if ((error = dlerror()) != NULL) {
		ALOGE(
				"Could not find address for function atrace_set_tracing_enabled: %s",
				error);
	}
}
}
...
virtual void onStarted() {
	...
	ALOGD("App process: starting thread pool.\n");
...
virtual void onZygoteInit() {
	...
	sp < ProcessState > proc = ProcessState::self();
	ALOGD("App process: starting thread pool. (onZygoteInit)\n");
	proc->startThreadPool();
}
...
if (argc == 2 && strcmp(argv[1], "--xposedtestsafemode") == 0) {
	...
	if (xposed::detectSafemodeTrigger (xposedSkipSafemodeDelay())) {printf("Safemode triggered\n");
} else {
...
if (zygote) {
	if (!xposedDisableSafemode()
			&& xposed::detectSafemodeTrigger(xposedSkipSafemodeDelay()))
		disableXposed();
}
...
keepLoadingDexspy = !isDexspyDisabled()
		&& !dexspyShouldIgnoreCommand(className, argc, argv)
		&& addDexspyToClasspath(zygote);
keepLoadingXposed = !isXposedDisabled()
		&& !xposedShouldIgnoreCommand(className, argc, argv)
		&& addXposedToClasspath(zygote);
ALOGD("keepLoadingXposed=%u keepLoadingDexspy=%u\n", keepLoadingXposed,
		keepLoadingDexspy);
